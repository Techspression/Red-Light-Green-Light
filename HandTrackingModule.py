...
class handDetector():
    ...
    def __init__(self,
        ...
        self.hands = self.mpHands.Hands(
            self.mode, self.maxHands, self.modelComplex, self.detectionCon,
    ...
    def findHands(self, img, draw=True):
        ...
        )  #converting to RGB bcoz hand recognition works only on RGB image
    ...
    def findPosition(self, img, handNo=0, draw=True):
        ...
        if self.results.multi_hand_landmarks:  # gives x,y,z of every landmark
            myHand = self.results.multi_hand_landmarks[
                handNo]  #Gives result for particular hand
            for id, lm in enumerate(myHand.landmark):  #gives id and lm(x,y,z)
                ...
                cx, cy = int(lm.x * w), int(
                    lm.y * h)  # pixels coordinates for landmarks
                # print(id, cx, cy)
                xList.append(cx)
                yList.append(cy)
    ...
    def fingersUp(self):  #checking which finger is open
        ...
        try:
            for id in range(
                ...
                if self.lmlist[self.tipIds[id]][2] < self.lmlist[
                        self.tipIds[id] - 2][2]:
                    fingers.append(1)
                else:
                    fingers.append(0)
    ...
    def findDistance(self,
                     ...
                     draw=True,
                     r=15,
                     t=3):  # finding distance between two points p1 & p2
...
def main():
    ...
    while True:
        ...
        cv2.putText(img, str(int(fps)), (10, 70), cv2.FONT_HERSHEY_COMPLEX, 3,
                    (255, 0, 255), 3)  # showing Fps on screen
